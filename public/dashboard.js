...
// --- Chart Interactivity ---
function setupChartInteractivity(options) {
  // Enable Pan/Zoom via chartjs-plugin-zoom
...
function renderRevenueChart(timeline) {
  ...
  setupChartInteractivity(customOptions);

  // Apply current divisions from the slider natively
  const divs = document.getElementById('timelineDivisions')?.value || 5;
  customOptions.scales.y.ticks = customOptions.scales.y.ticks || {};
  customOptions.scales.y.ticks.maxTicksLimit = parseInt(divs);
...
function updateRevenueDivisions() {
  if (revenueChartInstance) {
    const divs = document.getElementById('timelineDivisions').value;
    revenueChartInstance.options.scales.y.ticks.maxTicksLimit = parseInt(divs);
...
function renderProfitChart(timeline) {
  ...
  // Apply current divisions from the slider natively
  const divs = document.getElementById('profitDivisions')?.value || 5;
  customOptions.scales.y.ticks = customOptions.scales.y.ticks || {};
  customOptions.scales.y.ticks.maxTicksLimit = parseInt(divs);
...
function updateProfitDivisions() {
  if (profitChartInstance) {
    const divs = document.getElementById('profitDivisions').value;
    profitChartInstance.options.scales.y.ticks.maxTicksLimit = parseInt(divs);
...
function initializeGrid() {
  ...
  try {
    const savedLayout = localStorage.getItem("netigoGrid_v4");
    if (savedLayout) {
      grid.load(JSON.parse(savedLayout), true);
    }
  } catch (e) {
    console.warn('Corrupted grid layout, resetting:', e);
    localStorage.removeItem('netigoGrid_v4');
  }
...
function saveGrid() {
  ...
  localStorage.setItem("netigoGrid_v4", JSON.stringify(layout));
}
